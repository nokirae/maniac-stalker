...
- <code>/ms brand [бренд]</code> - Відображає/встановлює інформацію про бренд клієнта (для оновлення на сервері - слід перезайти);
...
- <code>/ms list</code> - Виводить інформацію про гравців (UUID, пінг (мережеву затримку), ігровий режим);
- <code>/ms toggle [модуль]</code> - Перемикає всі або вказаний модуль стеження (joinleave, gamemode, near);
- <code>/ms uwu</code> - Жартівлива команда для пошуку фембоїв та томбоїв на сервері (Працює краже якщо getTabListDisplayName() містить символ гендера (наприклад від плагіну HuGender)).
